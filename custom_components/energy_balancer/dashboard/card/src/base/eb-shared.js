...
export function semGetCurrency(hass) {
    if (!hass) return 'EUR';
    const costEntity = hass.states?.['sensor.ebi_daily_costs'];
 ...
 * updates (eb-flow-card, eb-system-diagram-card) call this in their
...
export function semDiscoverPVStrings(hass, prefix) {
    ...
    for (const slot of ['pv1', 'pv2', 'pv3', 'pv4']) {
        ...
        // fields; callers that show "today" (eb-solar-card's
 ...
 *
 * Why HTML-string and not Lit ``html`` literal here: ``eb-shared``
 * is the unscoped vanilla layer (eb-system-diagram-card et al. use
 ...
 * once. ``--eb-solar`` tints the value text; falls back to the
 * orange palette.
 */

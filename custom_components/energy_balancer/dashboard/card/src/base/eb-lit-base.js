...
import { semTheme } from './eb-shared.js';
...
export class SEMLitBase extends LitElement {
    ...
    set hass(hass) {
        ...
        // Static lists are written against the legacy 'ebi_' object prefix —
        // remap them onto the configured entity_prefix (_mapId is a no-op
        // when the prefix is the default). Cards may also provide an
        // instance-built list via this._watchedIds (already prefix-resolved).
        const watched = (this._watchedIds || this.constructor.watchedEntities || [])
            .map(id => this._mapId(id));
    ...
    // Map a raw forecast source id (sensor.ebi_forecast_source) to its
    ...
    // The cards were written against SEM's 'ebi_' object prefix. The EB
    // integration publishes 'eb_' entities; the configured entity_prefix
    // (default 'sensor.ebi_') decides which object prefix is live.
    // _objPrefix() strips the leading domain: 'sensor.eb_' -> 'eb_'.
    _objPrefix() {
        let p = this._prefix;
        if (typeof p === 'function') p = p.call(this);          // some cards define _prefix() as a method
        if (typeof p !== 'string' || !p) p = this._config?.entity_prefix || 'sensor.ebi_';
        const i = p.indexOf('.');
        return i >= 0 ? p.slice(i + 1) : p;
    }

    // Remap an entity id hardcoded with the legacy 'ebi_' object prefix
    // onto the configured one: 'switch.ebi_observer_mode' ->
    // 'switch.eb_observer_mode'. No-op for ids without the 'ebi_' prefix
    // and when the configured prefix is the default.
    _mapId(entityId) {
        const obj = this._objPrefix();
        if (obj === 'ebi_' || typeof entityId !== 'string') return entityId;
        return entityId.replace(/^([a-z_]+)\.ebi_/, `$1.${obj}`);
    }
    ...
    // 'number.ebi_daily_ev_target'). Charger entities follow the configured
    // object prefix (eb_ / ebi_), so build the discovery regex from it.
    _pcEntity(domain, suffix, globalFallback) {
        const st = this._hass?.states || {};
        const re = new RegExp(`^${domain}\\.${this._objPrefix()}charger_.+_${suffix}$`);
        ...
        match.sort();
        return match.length ? match[0] : this._mapId(globalFallback);
    }
    ...
    connectedCallback() {
        ...
        if (!this._localizeReady && typeof semLocalize !== 'function') {
            this._onLocalizeReady = () => {
                document.removeEventListener('eb-localize-ready', this._onLocalizeReady);
                this._onLocalizeReady = null;
                this._localizeReady = true;
                if (this._hass) this.requestUpdate();
            };
            document.addEventListener('eb-localize-ready', this._onLocalizeReady);

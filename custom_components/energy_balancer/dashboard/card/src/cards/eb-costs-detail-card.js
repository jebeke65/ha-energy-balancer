 ...
 *
 * Config:
 *   type: custom:eb-costs-detail-card
...
class SEMCostsDetailCard extends SEMLitBase {
    ...
    _sectionSubtitle(id, curr) {
        ...
        if (id === 'investment') {
            const inv = this._numVal(`number.${this._objPrefix()}system_investment_cost`);
            return inv > 0 ? `${inv.toFixed(0)} ${curr}` : '';
        }
        if (id === 'demand') {
            const peak = this._val('monthly_consecutive_peak');
            const charge = this._val('power_charge_cost');
            // peak is in W (sensor device_class=power). semFormatPower converts;
            // `${peak.toFixed(1)} kW` printed 2941 W as "2941.0 kW".
            return `${semFormatPower(peak)} · ${charge.toFixed(2)} ${curr}`;
        }
        if (id === 'tariff') {
    ...
    _renderSectionBody(id, curr) {
        ...
        if (id === 'investment') {
            return html`${this._renderStepper(`number.${this._objPrefix()}system_investment_cost`, 'system_investment_cost')}`;
        }
        ...
        if (id === 'demand') {
            ...
            return html`
                <div class="info-tiles">
                    ${this._renderInfoTile('mdi:chart-bell-curve', '#ff9800', 'monthly_peak', semFormatPower(peak))}
        ...
        if (id === 'tariff') {
            const impRateEnt = this._hass?.states[`${this._prefix}tariff_current_import_rate`];
            const expRateEnt = this._hass?.states[`${this._prefix}tariff_current_export_rate`];
            ...
            const hasStaticImport = this._entityExists(`number.${this._objPrefix()}electricity_import_rate`);
            const hasStaticExport = this._entityExists(`number.${this._objPrefix()}electricity_export_rate`);
            ...
            return html`
                ...
                ${hasStaticImport ? this._renderStepper(`number.${this._objPrefix()}electricity_import_rate`, 'current_import_rate') : nothing}
                ${hasStaticExport ? this._renderStepper(`number.${this._objPrefix()}electricity_export_rate`, 'current_export_rate') : nothing}
            `;
        }
...
semDefineCard('eb-costs-detail-card', SEMCostsDetailCard, {
    type: 'custom:eb-costs-detail-card',

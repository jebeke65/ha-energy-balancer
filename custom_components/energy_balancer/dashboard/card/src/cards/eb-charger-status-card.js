 ...
 *
 * Config:
 *   type: custom:eb-charger-status-card
    ...
    set hass(hass) {
        ...
        if (stateCount !== this._lastStateCount) {
            ...
            for (const eid of Object.keys(hass.states)) {
                // #356 — same ghost-charger trap as eb-ev-status-card: the
                // per-charger flow sensors (sensor.ebi_charger_<id>_flow_*_power)
                // match the regex below. Filter them out before extracting the id.
                if (eid.includes('_flow_')) continue;
                const match = eid.match(/^sensor\.ebi_charger_(.+)_power$/);
...
semDefineCard('eb-charger-status-card', SEMChargerStatusCard, {
    type: 'eb-charger-status-card',

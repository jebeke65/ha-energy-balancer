 ...
 * Listens for 'eb-period-change' events from eb-period-selector-card.
...
import { semTheme, semGetCurrency, semDefineCard, SEM_COLORS } from '../base/eb-shared.js';
...
class SEMChartCard extends SEMLitBase {
    ...
    setConfig(config) {
        if (!config.preset && !config.series) {
            throw new Error('eb-chart-card requires either preset or series config');
        }
        this._config  = config;
    ...
    connectedCallback() {
        ...
        document.addEventListener('eb-period-change', this._boundPeriodHandler);
    ...
    disconnectedCallback() {
        ...
        document.removeEventListener('eb-period-change', this._boundPeriodHandler);
    ...
    _setDefaultPeriod() {
        ...
        // default_period must work for series-only cards too (no preset),
        // so don't gate the hourly/24h view on a preset existing.
        const isHourly = wantToday || defaultPeriod === '24h' || (p && p.hourly && !p.daily);
        if (isHourly) {
    ...
    _discoverPerChargerSeries() {
        ...
        const re = /^sensor\.ebi_charger_(.+)_power$/;
        const found = [];
    ...
    async _fetchAndRender() {
        ...
        try {
            ...
            console.debug('eb-chart-card: fetch error', err);
...
semDefineCard('eb-chart-card', SEMChartCard, {
    type: 'eb-chart-card',
    name: 'SEM Chart',

...
import { semTheme, semDefineCard } from '../base/eb-shared.js';

const ZONES = [
    { id: 'autostart', entity: 'number.ebi_battery_auto_start_soc', icon: 'mdi:play-circle',        labelKey: 'auto_start_soc', helpKey: 'zone_help_autostart', color: '#4db6ac' },
    { id: 'buffer',    entity: 'number.ebi_battery_buffer_soc',    icon: 'mdi:shield-half-full',    labelKey: 'buffer_soc',    helpKey: 'zone_help_buffer',    color: '#ff9800' },
    { id: 'priority',  entity: 'number.ebi_battery_priority_soc',  icon: 'mdi:shield-alert',       labelKey: 'priority_soc',  helpKey: 'zone_help_priority', color: '#f44336' },
];
...
class SEMBatteryZonesCard extends SEMLitBase {
    ...
    /** Zone entity id resolved onto the configured entity prefix. */
    _zoneEntity(z) {
        return this._mapId(z.entity);
    }

    _renderZoneMarkers(T) {
        return ZONES.map(z => {
            const val = this._state(this._zoneEntity(z));
    ...
    _renderStepper(z, T) {
        const entityId = this._zoneEntity(z);
        const val = this._state(entityId);
        const decimals = this._getDecimalsForZone(entityId);
    ...
    render() {
        ...
        const T = this._theme();
        const autostart = this._state(this._zoneEntity(ZONES[0]));
        const buffer    = this._state(this._zoneEntity(ZONES[1]));
        const priority  = this._state(this._zoneEntity(ZONES[2]));
...
semDefineCard('eb-battery-zones-card', SEMBatteryZonesCard, {
    type: 'custom:eb-battery-zones-card',

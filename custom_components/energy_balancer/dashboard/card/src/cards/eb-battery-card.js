 ...
 * Matches the Lumina-styled design from the original eb-battery-card.js exactly,
 * but uses Lit reactive rendering instead of innerHTML + manual DOM patching.
 *
 * Config:
 *   type: custom:eb-battery-card
...
class SEMBatteryCard extends SEMLitBase {
    ...
    set hass(hass) {
        ...
        if (stateCount !== this._lastStateCount) {
            ...
            for (const eid of Object.keys(hass.states)) {
                const m = eid.match(new RegExp(`^sensor\\.${this._objPrefix()}battery_(b\\d+)_power$`));
        ...
        if (this._batteries.length) {
            key += '|' + this._batteries.map(bid => {
                ...
                const ctrl = [
                    `select.${this._objPrefix()}battery_${bid}_mode`,
                    `number.${this._objPrefix()}battery_${bid}_reserve_soc`,
            ...
            key += '|' + [
                `select.${this._objPrefix()}battery_mode`,
                `number.${this._objPrefix()}battery_reserve_soc`,
            ].map(e => hass?.states[e]?.state || '').join(':');
        }
    ...
    // True only when the backing entity exists with a usable state.
    _exists(suffix) {
        const s = this._hass?.states?.[`${this._prefix}${suffix}`];
    ...
    render() {
        ...
        const exportEnt = this._hass?.states[`${this._prefix}tariff_current_export_rate`];
...
semDefineCard('eb-battery-card', SEMBatteryCard, {
    type: 'eb-battery-card',
    name: 'SEM Battery',

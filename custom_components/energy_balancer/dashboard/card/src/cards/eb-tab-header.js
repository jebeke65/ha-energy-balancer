...
class SEMTabHeader extends SEMLitBase {
    ...
    // The sensor suffix behind each stat tile, parallel to labels/values, so a tap
    // opens that entity's more-info dialog. Kept in step with _buildStatsKey above.
    _getStatEntities() {
        const tab = this._tab;
        if (tab === 'home')    return ['solar_power', 'autarky_rate', 'self_consumption_rate', 'daily_solar_energy'];
        if (tab === 'energy')  return ['daily_solar_energy', 'daily_home_energy', 'self_consumption_rate'];
        if (tab === 'battery') return ['battery_soc', 'battery_power', 'battery_health_score'];
        if (tab === 'ev')      return ['ev_power', 'daily_ev_energy', 'charging_state'];
        if (tab === 'control') return ['target_peak_limit', 'controllable_devices_count', 'surplus_active_devices'];
        if (tab === 'config')  return ['charging_state', 'heat_pump_registered', 'battery_status'];
        if (tab === 'costs')   return ['daily_costs', 'daily_savings', 'daily_net_cost'];
        if (tab === 'system')  return ['energy_optimization_score', 'lifetime_total_savings', 'lifetime_co2_avoided'];
        return [];
    }

    _openStat(suffix) {
        const entityId = `${this._prefix}${suffix}`;
        if (!this._hass || !this._hass.states[entityId]) return;   // no entity → no dialog
    ...
    _getStatValues() {
        ...
        if (tab === 'battery') return [
            ...
            this._exists('battery_health_score')
                ? this._getState('battery_health_score', 0).toFixed(0) + '%'
                : '—',
        ];
        if (tab === 'ev') return [
        ...
        if (tab === 'control') return [
            // target_peak_limit is in W; " kW" on the raw value showed 3050 W as "3050.0 kW".
            semFormatPower(this._getState('target_peak_limit', 0)),
        ...
        if (tab === 'config') {
            ...
            const chargers = Object.keys(this._hass?.states || {})
                .filter(e => e.match(/^number\.ebi_charger_.+_minimum_current$/)).length;
    ...
    render() {
        ...
        const entities = this._getStatEntities();
        const T = this._theme();
        ...
        return html`
            <style>
                ...
                .stat.clickable:hover { background: ${T.dotColor}; }
                .stat-value {
            ...
            <ha-card>
                <div class="header-wrap">
                    ...
                    <div class="stats">
                        ${labels.map((label, i) => {
                            const suffix = entities[i];
                            const has = suffix && this._hass.states[`${this._prefix}${suffix}`];
                            return html`
                            <div class="stat ${has ? 'clickable' : ''}"
                                 @click=${has ? () => this._openStat(suffix) : null}>
...
semDefineCard('eb-tab-header', SEMTabHeader, {
    type: 'eb-tab-header',
    name: 'SEM Tab Header',

 ...
 *
 * #492: settings de-duplicated to eb-config-card — this card is the
 ...
 *
 * Config:
 *   type: custom:eb-control-card
 *   entity_prefix: sensor.ebi_   # optional, default
 */
...
const SECTIONS = [
    ...
    // Hot-water and heat-pump sections removed for EB — no hot-water or
    // heat-pump controller in this system (2026-07-10, user request).
    {
    ...
    },
    {
        ...
        subtitleFn: (c) => {
            // Both sensors are in W. Appending " kW" to the raw value showed a
            // 2941 W monthly peak as "2941.0 kW".
            const peak = semFormatPower(c._valNum('consecutive_peak_15min'));
            const monthly = semFormatPower(c._valNum('monthly_consecutive_peak'));
            return `${c._t('peak')} ${peak} · ${c._t('monthly_peak')} ${monthly}`;
        },
    },
];
 ...
 * the EV tab via eb-ev-status-card).
 *
 * Pruned again in #492: the 10 settings duplicated on eb-config-card
 * (SOC steppers, price thresholds, solar/grid limits, boost offset,
 * solar-target, observer toggle) render only there now, so their
 * number.* entities are no longer read here. switch.ebi_observer_mode
 * stays — the live observer-warning banner reads it.
 */
const WATCHED = [
    'sensor.ebi_surplus_active_devices', 'sensor.ebi_surplus_total_devices',
    'sensor.ebi_surplus_allocated_w', 'sensor.ebi_surplus_total_w',
    'sensor.ebi_surplus_distributable_w', 'sensor.ebi_surplus_unallocated_w',
    'sensor.ebi_battery_soc', 'sensor.ebi_battery_status',
    'sensor.ebi_diag_battery_capacity',
    'number.ebi_legionella_target_temp',
    // #437 — heat pump section: registered? mode, sg_ready_state
    'binary_sensor.ebi_heat_pump_registered',
    'sensor.ebi_heat_pump_sg_ready_state', 'sensor.ebi_heat_pump_mode',
    'binary_sensor.ebi_heat_pump_solar_boost',
    'sensor.ebi_tariff_provider', 'sensor.ebi_tariff_price_level',
    'sensor.ebi_tariff_current_import_rate',
    'sensor.ebi_consecutive_peak_15min', 'sensor.ebi_monthly_consecutive_peak',
    'sensor.ebi_current_vs_peak_percentage', 'sensor.ebi_load_management_status',
    'sensor.ebi_load_management_recommendation',
    'sensor.ebi_peak_margin', 'sensor.ebi_available_load_reduction',
    'sensor.ebi_controllable_devices_count',
    'switch.ebi_observer_mode',
];
...
class SEMControlCard extends SEMLitBase {
    ...
    /** Numeric state for a number.<objPrefix> suffixed entity. */
    _numVal(suffix, fallback = 0) {
        const e = this._hass?.states[`number.${this._objPrefix()}${suffix}`];
        if (!e || e.state === 'unavailable' || e.state === 'unknown') return fallback;
        return parseFloat(e.state) ?? fallback;
    }

    /** Boolean: is switch.<objPrefix><suffix> on? */
    _switchOn(suffix) {
        return this._hass?.states[`switch.${this._objPrefix()}${suffix}`]?.state === 'on';
    }

    /** Boolean: is binary_sensor.<objPrefix><suffix> on? (#523)
     * ``_val()`` prepends the sensor prefix so it silently returns '' for
    ...
    _bin(suffix) {
        return this._hass?.states[`binary_sensor.${this._objPrefix()}${suffix}`]?.state === 'on';
    }
    ...
    _renderHeatPumpSection(T) {
        ...
        // via _bin(), not _val() (which prepends sensor.ebi_ and always
    ...
    _renderHotWaterSection(T) {
        // #492: solar_boost_target moved to eb-config-card. The
    ...
    _renderTariffSection(T) {
        const rateEntity = this._hass?.states[`${this._prefix}tariff_current_import_rate`];
            ...
            <div class="info-tiles">
                <div class="info-tile">
                    ...
                    <div class="info-tile-content">
                        ...
                        <span class="info-tile-value">${semFormatPower(this._valNum('peak_margin'))}</span>
                    </div>
                </div>
                <div class="info-tile">
                    ...
                    <div class="info-tile-content">
                        ...
                        <span class="info-tile-value">${semFormatPower(shed)} · ${shedDevices} ${this._t('devices')}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    render() {
        ...
        return html`
            <style>
                ...
                .eb-select:focus { border-color: ${accent}; }
                .eb-select option {
    ...
    static getStubConfig() {
        return { entity_prefix: 'sensor.ebi_' };
    }
}

semDefineCard('eb-control-card', SEMControlCard, {
    type: 'custom:eb-control-card',

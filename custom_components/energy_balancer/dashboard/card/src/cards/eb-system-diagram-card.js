 ...
 *   - render() — reads state inline each cycle (matches eb-battery-card.js
...
class SEMSystemDiagramCard extends SEMLitBase {
     ...
     * Entities mode: the eb-flow-card schema map — unmapped keys
    ...
    //    eb-flow-card (split battery, combined grid, reverse/invert
    ...
    _flowHome(solar, gridImport, gridExport, battCharge, battDischarge, ev) {
        ...
        // EV charges). ``sensor.ebi_home_consumption_power`` carries the
    ...
    set hass(hass) {
        ...
        // matches eb-battery-card.js:89-92. ~600ms window after every cycle.
        ...
        key += '|' + (hass?.states[`binary_sensor.${this._objPrefix()}ev_connected`]?.state || '');
        key += '|' + (hass?.states[`binary_sensor.${this._objPrefix()}ev_charging`]?.state || '');
    ...
    // ── Suffix-scoped state readers (matches eb-battery-card.js:116-122;
    ...
    render() {
        ...
        // each bounce flips ``sensor.ebi_battery_soc`` and ``sensor.ebi_battery_power``
        ...
        const evConnected = this._hass?.states[`binary_sensor.${this._objPrefix()}ev_connected`]?.state === 'on';
        const evCharging  = this._hass?.states[`binary_sensor.${this._objPrefix()}ev_charging`]?.state === 'on';
...
semDefineCard('eb-system-diagram-card', SEMSystemDiagramCard, {
    type: 'eb-system-diagram-card',

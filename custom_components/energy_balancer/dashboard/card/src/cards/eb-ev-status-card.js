 ...
 *
 * Config:
 *   type: custom:eb-ev-status-card
 *   entity_prefix: sensor.ebi_   # default
 */

import { SEMLitBase, html, css, svg, nothing } from '../base/eb-lit-base.js';
import { semTheme, semFormatPower, semGetCurrency, semDefineCard } from '../base/eb-shared.js';

const DEFAULT_PREFIX = 'sensor.ebi_';
...
class SEMEVStatusCard extends SEMLitBase {
    ...
    set hass(hass) {
        ...
        if (stateCount !== this._lastStateCount) {
            ...
            for (const eid of Object.keys(hass.states)) {
                // #356 — per-charger flow sensors (sensor.ebi_charger_<id>_flow_*_power)
                ...
                const match = eid.match(new RegExp(`^sensor\\.${this._objPrefix()}charger_(.+)_power$`));
        ...
        let key = [
            ...
            const pfx = (s === 'ev_connected' || s === 'ev_charging')
                ? `binary_sensor.${this._objPrefix()}` : prefix;
        ...
        if (this._chargers.length >= 1) {
            ...
            key += '|' + this._chargers.map(id =>
                hass.states[`switch.${this._objPrefix()}charger_${id}_night_charging`]?.state || ''
            ).join(':');

            // Deadline (#246) + tariff (#247) entities
            key += '|' + this._chargers.map(id => [
                hass.states[`time.${this._objPrefix()}charger_${id}_target_time`]?.state || '',
                hass.states[`switch.${this._objPrefix()}charger_${id}_tariff_optimized`]?.state || '',
            ].join(':')).join('|');
            ...
            key += '|' + this._chargers.map(id =>
                hass.states[`number.${this._objPrefix()}charger_${id}_daily_ev_target`]?.state || ''
            ).join(':');

            // Charge Target range (#245): type, SOC floor, both Max ceilings, capacity
            key += '|' + this._chargers.map(id => [
                hass.states[`select.${this._objPrefix()}charger_${id}_ev_target_type`]?.state || '',
                hass.states[`number.${this._objPrefix()}charger_${id}_target_soc`]?.state || '',
                hass.states[`number.${this._objPrefix()}charger_${id}_daily_ev_target_max`]?.state || '',
                hass.states[`number.${this._objPrefix()}charger_${id}_target_soc_max`]?.state || '',
                hass.states[`number.${this._objPrefix()}charger_${id}_ev_battery_capacity_kwh`]?.state || '',
                hass.states[`number.${this._objPrefix()}charger_${id}_ev_kwh_per_100km`]?.state || '',
            ].join(':')).join('|');
    ...
    _binaryState(suffix) {
        const e = this._hass?.states[`binary_sensor.${this._objPrefix()}${suffix}`];
        return e?.state === 'on';
    }
     ...
     * plan rows on ebi_charging_state. Walks the plan rows and paints a
    ...
    _renderPlanStrip(chargerId) {
        const cs = this._hass?.states[`${this._prefix}charging_state`];
    ...
    _renderChargerSection(id, idx) {
        ...
        // charger ``sensor.ebi_charger_<id>_vehicle_soc`` now exists
        // (#383) — each card reads its own charger's SOC directly
        // instead of falling back to the global ``ebi_vehicle_soc``
        ...
        const perChargerConnected = this._hass?.states[`binary_sensor.${this._objPrefix()}charger_${id}_connected`];
        ...
        const minAmps = this._entityVal(`number.${this._objPrefix()}charger_${id}_minimum_current`, 6);
        const capacityKwh = this._entityVal(`number.${this._objPrefix()}charger_${id}_ev_battery_capacity_kwh`, 40);
        const consumption = this._entityVal(`number.${this._objPrefix()}charger_${id}_ev_kwh_per_100km`, 18);
        ...
        const chargeModeEntityId = `select.${this._objPrefix()}charger_${id}_charge_mode`;
        ...
        const bufferSoc = Math.round(
            this._entityVal(`number.${this._objPrefix()}battery_buffer_soc`, 70));
        const prioritySoc = Math.round(
            this._entityVal(`number.${this._objPrefix()}battery_priority_soc`, 30));
        ...
        const targetTypeId = `select.${this._objPrefix()}charger_${id}_ev_target_type`;
        ...
        const minEntityId = isSoc
            ? `number.${this._objPrefix()}charger_${id}_target_soc`
            : `number.${this._objPrefix()}charger_${id}_daily_ev_target`;
        const maxEntityId = isSoc
            ? `number.${this._objPrefix()}charger_${id}_target_soc_max`
            : `number.${this._objPrefix()}charger_${id}_daily_ev_target_max`;

        // Deadline (#246) — the tariff/grid toggles moved into the
        // Charge mode selector above, but the deadline knob remains.
        const targetTimeId = `time.${this._objPrefix()}charger_${id}_target_time`;
                ...
                <div class="charger-settings ${this._showHelp ? 'help-mode' : ''}">
                    ...
                    <div class="setting-cell">
                        <div
                            ...
                            @click=${() => {
                                const event = new CustomEvent('hass-more-info', { bubbles: true, composed: true, detail: { entityId: `number.${this._objPrefix()}charger_${id}_minimum_current` } });
                    ...
                    <div class="setting-cell">
                        <div
                            class="setting-item clickable"
                            @click=${() => {
                                const event = new CustomEvent('hass-more-info', { bubbles: true, composed: true, detail: { entityId: `number.${this._objPrefix()}charger_${id}_ev_battery_capacity_kwh` } });
                                this.dispatchEvent(event);
                            }}
                        >
                    ...
                    <div class="setting-cell">
                        <div
                            class="setting-item clickable"
                            @click=${() => {
                                const event = new CustomEvent('hass-more-info', { bubbles: true, composed: true, detail: { entityId: `number.${this._objPrefix()}charger_${id}_ev_kwh_per_100km` } });
                                this.dispatchEvent(event);
                            }}
                        >
...
semDefineCard('eb-ev-status-card', SEMEVStatusCard, {
    type: 'eb-ev-status-card',
    name: 'SEM EV Status',

 ...
 *
 * Config:
 *   type: custom:eb-system-card
 *   entity_prefix: sensor.ebi_   # optional, default
 */

import { SEMLitBase, html, css, nothing } from '../base/eb-lit-base.js';
import { semTheme, semFormatPower, semGetCurrency, semCardSurfaceCSS, SEM_COLORS, semDefineCard } from '../base/eb-shared.js';
 ...
 * (ebi_forecast_today → ebi_forecast_today_kwh; ebi_night_mode,
 * ebi_solar_mode, ebi_night_mode_status, ebi_solar_mode_status had no
 * corresponding sensor.py keys; ebi_ev_connected lives under binary_sensor).
 * Replaced ebi_daily_ev_target (sensor) with the actual number entity
 * ebi_daily_ev_target_global. Added ebi_charging_state since the card reads
 * its today_plan attribute downstream.
 */
const WATCHED = [
    'sensor.ebi_diag_version', 'sensor.ebi_diag_grid_mode',
    'sensor.ebi_diag_battery_capacity', 'sensor.ebi_diag_update_interval',
    'sensor.ebi_diag_charger_count', 'sensor.ebi_diag_charger_control',
    'sensor.ebi_diag_sensors_unavailable', 'sensor.ebi_diag_ed_config',
    'sensor.ebi_solar_power', 'sensor.ebi_grid_power',
    'sensor.ebi_battery_soc', 'sensor.ebi_ev_power',
    'sensor.ebi_forecast_today_kwh', 'sensor.ebi_tariff_current_import_rate',
    'switch.ebi_observer_mode',
    'sensor.ebi_home_consumption_power', 'sensor.ebi_grid_import_power',
    'sensor.ebi_grid_export_power', 'sensor.ebi_autarky_rate',
    'sensor.ebi_self_consumption_rate', 'sensor.ebi_battery_power',
    'sensor.ebi_night_charging_status',
    'sensor.ebi_battery_status', 'sensor.ebi_grid_status',
    'sensor.ebi_charging_state',
];
...
class SEMSystemCard extends SEMLitBase {
    ...
    _copyDiagnostics() {
        ...
        const ed = this._hass?.states?.['sensor.ebi_diag_ed_config']?.attributes?.energy_dashboard;
        let edBlock;
        if (ed) {
            const v = x => x || '—';
            edBlock =
...
semDefineCard('eb-system-card', SEMSystemCard, {
    type: 'custom:eb-system-card',

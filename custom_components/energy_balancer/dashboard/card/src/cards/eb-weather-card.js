...
class SEMWeatherCard extends SEMLitBase {
    ...
    setConfig(config) {
        if (!config.entity) throw new Error('eb-weather-card requires entity');
    ...
    _updateClock() {
        const now = new Date();
        // Reference = the HOST's configured regional locale (language + country), so the
        // clock follows the home's region (e.g. BE -> 24h) instead of the UI language alone.
        const cfg = this._hass?.config;
        const baseLang = (cfg?.language || this._hass?.language || navigator.language || 'en').split('-')[0];
        const country = cfg?.country;
        const locale = country ? `${baseLang}-${country}` : baseLang;
        // Show the home's local time (HA tz, DST-aware), not the viewer's browser tz.
        const tz = cfg?.time_zone || undefined;
        // An explicit HA time-format setting wins; otherwise let the regional locale decide.
        const tf = this._hass?.locale?.time_format;
        const timeOpts = { hour: '2-digit', minute: '2-digit', timeZone: tz };
        if (tf === '24') timeOpts.hour12 = false;
        else if (tf === 'am_pm' || tf === '12') timeOpts.hour12 = true;
        this._clockTime = now.toLocaleTimeString(locale, timeOpts);
...
semDefineCard('eb-weather-card', SEMWeatherCard, {
    type: 'custom:eb-weather-card',
    name: 'SEM Weather',

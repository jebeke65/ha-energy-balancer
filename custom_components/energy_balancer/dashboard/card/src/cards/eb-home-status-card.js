 ...
 *
 * Config:
 *   type: custom:eb-home-status-card
 *   entity_prefix: sensor.ebi_   # optional, default
 */

import { SEMLitBase, html, css, nothing } from '../base/eb-lit-base.js';
import { semTheme, semFormatPower, semDefineCard } from '../base/eb-shared.js';

const DEFAULT_PREFIX = 'sensor.ebi_';

const WATCHED = [
    'sensor.ebi_solar_power',
    'sensor.ebi_battery_soc',
    'sensor.ebi_autarky_rate',
    'sensor.ebi_ev_power',
    'sensor.ebi_energy_optimization_score',
    'sensor.ebi_energy_tip',
    'sensor.ebi_best_surplus_window',
    'sensor.ebi_forecast_remaining_today_kwh',
    'sensor.ebi_current_vs_peak_percentage',
    'sensor.ebi_consecutive_peak_15min',
    'sensor.ebi_target_peak_limit',
    'sensor.ebi_daily_co2_avoided',
    'sensor.ebi_lifetime_co2_avoided',
    'sensor.ebi_forecast_source',
...
class SEMHomeStatusCard extends SEMLitBase {
    ...
    // Open the entity behind a chip. No-op when the sensor does not exist, so a chip
    // backed by a not-yet-published field just stays inert instead of opening a blank.
    _openEntity(suffix) {
    ...
    _renderChip(icon, color, value, suffix) {
        const has = suffix && this._hass?.states[`${this._prefix}${suffix}`];
        return html`
            <div class="chip ${has ? 'clickable' : ''}"
                 style=${has ? 'cursor:pointer' : ''}
                 @click=${has ? () => this._openEntity(suffix) : null}
                 title=${has ? `${this._prefix}${suffix}` : ''}>
    ...
    render() {
        ...
        const currentPeak = semFormatPower(this._val('consecutive_peak_15min'));
        const targetLimit = semFormatPower(this._val('target_peak_limit'));

        const observerOn = this._switchOn(`switch.${this._objPrefix()}observer_mode`);
        // Friendly label for the forecast provider. The coordinator emits
        // raw source-IDs ("solcast", "forecast_solar", "custom") via
        // sensor.ebi_forecast_source — they're brand names mostly, so we
        ...
        return html`
            <div class="wrap">
                ...
                <!-- 1. Status Chips -->
                <div class="chips-row">
                    ${this._renderChip('mdi:solar-power', '#ff9800', semFormatPower(solar), 'solar_power')}
                    ${this._renderChip('mdi:battery', '#4db6ac', `${soc.toFixed(0)}%`, 'battery_soc')}
                    ${this._renderChip('mdi:leaf', '#8DC892', `${autarky.toFixed(0)}%`, 'autarky_rate')}
                    ${this._renderChip('mdi:car-electric', '#8DC892', evPower > 0 ? semFormatPower(evPower) : '—', 'ev_power')}
                    ${this._renderChip('mdi:speedometer', scoreColor, score.toFixed(0), 'energy_optimization_score')}
                </div>
                ...
                <div class="peak-row">
                    ...
                    <div class="peak-text-wrap">
                        <span class="peak-detail">${currentPeak} / ${targetLimit} (${peakPct.toFixed(0)}%)</span>
...
semDefineCard('eb-home-status-card', SEMHomeStatusCard, {
    type: 'custom:eb-home-status-card',

...
import { semTheme, semDefineCard, semFormatPower } from '../base/eb-shared.js';
import {
    ENTITY_DOMAINS,
    controlToFormValues,
...
class SEMLoadPriorityCard extends SEMLitBase {
    constructor() {
        super();
        this.devices = [];
        this.targetPeakLimit = 0;    // W — EB publishes power in W, not kW
        this.currentPeak = 0;
    ...
    setConfig(config) {
        this._config      = config;
        this.entityPrefix = config.entity_prefix || 'sensor.ebi_';
        this.requestUpdate();
    }
    ...
    _updateDeviceData() {
        ...
        if (targetPeakEntity)  this.targetPeakLimit      = parseFloat(targetPeakEntity.state)  || 0;
            ...
            <ha-card>
                <div class="card-content">
                    <div class="status-bar">
                        ...
                        <span id="peak-current" class="mono">${semFormatPower(this.currentPeak)}</span>
                        <span class="dim">/ ${semFormatPower(this.targetPeakLimit)}</span>
                    </div>

                    <div class="peak-box">
                        <div class="peak-row">
                            <span class="dim">${this._t('current_peak')}</span>
                            <span id="peak-current2" class="mono">${semFormatPower(this.currentPeak)}</span>
                        </div>
                        <div class="peak-row">
                            <span class="dim">${this._t('target_limit')}</span>
                            <span id="peak-target" class="mono">${semFormatPower(this.targetPeakLimit)}</span>
                        </div>
                        <div class="peak-row">
                            <span class="dim">${this._t('margin')}</span>
                            <span id="peak-margin" class="mono" style="color:${peakMargin > 0 ? '#4caf50' : '#f44336'}">${semFormatPower(peakMargin)}</span>
                        </div>
                        <div class="bar">
    ...
    _sendTargetPeakUpdate(val) {
        if (!this._hass) return;
        // Was solar_energy_management.update_target_peak — a SEM service that does
        // not exist here, so the Set button failed silently. EB exposes the limit as
        // a number entity, in W.
        // entity_prefix is "sensor.eb_"; the number entity lives under the same slug.
        const slug = String(this.entityPrefix || 'sensor.eb_').replace(/^sensor\./, '');
        this._hass.callService('number', 'set_value', {
            entity_id: `number.${slug}peak_limit_w`,
    ...
    async _showConfigureModal(energySensor, deviceName) {
        const existing = this.renderRoot.getElementById('eb-config-modal');
...
semDefineCard('eb-load-priority-card', SEMLoadPriorityCard, {
    type: 'eb-load-priority-card',

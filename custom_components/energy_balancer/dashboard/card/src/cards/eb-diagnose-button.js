 ...
 *
 * Usage:
 *   <eb-diagnose-button section="heat_pump" label="Diagnose"></eb-diagnose-button>
 *
...
import { semDefineCard } from '../base/eb-shared.js';
...
class SEMDiagnoseButton extends SEMLitBase {
    ...
    async _click() {
        ...
        try {
            ...
            console.error('[eb-diagnose-button] failed', err);
        } finally {
            this._busy = false;
        }
    }
    ...
    async _copy() {
        ...
        // Pattern mirrors eb-system-card._writeClipboard (#285).
        if (navigator?.clipboard?.writeText && window.isSecureContext !== false) {
            try {
                ...
                console.warn('[eb-diagnose-button] modern clipboard refused, '
        ...
        try {
            ...
            console.error('[eb-diagnose-button] execCommand("copy") returned false');
        } catch (err) {
            console.error('[eb-diagnose-button] legacy clipboard fallback threw', err);
        }
...
if (!customElements.get('eb-diagnose-button')) {
    customElements.define('eb-diagnose-button', SEMDiagnoseButton);
}
